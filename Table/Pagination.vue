    ...
    methods: {
        goToNextPage: function(){
            if (!this.nextBtnDisabled) {
                this.$Progress.start();
                fetch(this.pagination['hydra:next']).then(response => {
                    return response.json()
                }).then(data => {
                    this.$Progress.finish();
                    let pageNumber = parseInt(this.pagination['hydra:next'].slice(this.pagination['hydra:next'].lastIndexOf('=') + 1));
        ...
        goToPreviousPage: function(){
            if (!this.previousBtnDisabled) {
                this.$Progress.start();
                fetch(this.pagination['hydra:previous']).then(response => {
                    return response.json()
                }).then(data => {
                    this.$Progress.finish();
                    let pageNumber = parseInt(this.pagination['hydra:previous'].slice(this.pagination['hydra:previous'].lastIndexOf('=') + 1));
                    EventBus.$emit('paginationAction', data);
                })
            }
        },
        goToPage: function(pageNumber) {
            if (pageNumber !== this.activePage && (pageNumber>=this.firstPage && pageNumber<=this.lastPage)) {
                let url = this.pagination['@id'].slice(0, this.pagination['@id'].lastIndexOf('=')) + "=" + pageNumber;
                this.$Progress.start();
